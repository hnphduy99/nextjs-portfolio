...
const useAnimationFrame = (callback: (time: number, delta: number) => void) => {
  ...
  // Update ref to the latest callback on every render
  useEffect(() => {
...
function Marquee({
  ...
  return (
    <div
      ...
      ref={containerRef}
      className={cn(
        "group flex gap-(--gap) overflow-hidden p-2 [--gap:2rem]" + (vertical ? " flex-col" : " flex-row"),
        className
      )}
    ...
    >
      <div
        ref={contentRef}
        className={cn("flex shrink-0 justify-around gap-(--gap)" + (vertical ? " flex-col" : " flex-row"))}
      >
        {Array(repeat)
          .fill(0)
          .map((_, i) => (
